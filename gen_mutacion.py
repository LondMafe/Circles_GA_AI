...
def genetic_algorithm(circles_data, canvas_width, canvas_height, mutation_probability):
    ...
    for generation in range(generations):
        ...
        for i in range(len(offspring)):
            if random.random() < mutation_probability:  # Usa la probabilidad de mutación modificada
    ...
    for i in range(10):
        image.save(f"genetic_algorithm_run_{prob}.png" )
...
# Ejecutar el algoritmo con diferentes probabilidades de mutación
probabilities = [0.05, 0.2]  # Por ejemplo, cambiar la probabilidad de mutación a 5% y luego a 20%
results = []
for prob in probabilities:
    best_circle, best_fitness = genetic_algorithm(circles_data, canvas_width, canvas_height, prob)
    results.append((prob, best_circle, best_fitness))

# Imprime los resultados para comparar
for prob, circle, fitness in results:
    print(f"Mutation Probability {prob}: Best Circle = {circle}, Fitness = {fitness}")